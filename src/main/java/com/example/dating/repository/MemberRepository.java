...
public interface MemberRepository extends JpaRepository<Member, Long> {
    ...
    @Query("select m from Member m where not m.gender = :gender " +
            "and m.id not in (select h.receiver.id from Heart h where h.sender.id = :id) " +
            "and m.id not in (select h.sender.id from Heart h where h.receiver.id = :id) " +
            "and m.id not in (select hm.member.id from HumanMember hm) " +
            "and m.id not in (SELECT b.blockMember.id FROM Block b WHERE b.blockItMember.id = :id) order by rand()")
    List<Member> findRandomMember(@Param("id") Long id, @Param("gender") String gender, Pageable pageable);
...
//    @Query(value = "SELECT m FROM Member m LEFT JOIN heart h ON (h.sender = )  WHERE NOT m.gender = :gender")
//    List<Member> findRandomRecommendMemberList(@Param("gender") String gender, @Param("id") Long id, Pageable pageable);
